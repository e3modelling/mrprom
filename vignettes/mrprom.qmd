...
The function disaggregates STEAM into their corresponding energy forms (EFs)<br>
(e.g. STE1AH) for the industrial, domestic and non-energy use sector.<br>
First, STEAM shares per EF are extracted from IEA2025 data subsets c("HEMAINC", "HEAUTOC").<br>
HEMAINC: Heat output (TJ)-main activity producer CHP plants<br>
HEAUTOC: Heat output (TJ)-autoproducer CHP plants<br>
Sectoral mappings (DOMSE, INDSE, NENSE) and auxiliary mappings for CHP and<br>
district heating (DH) to EFs are then combined to create a comprehensive<br>
EF-to-sector mapping. The function applies these shares to disaggregate STEAM<br>
into detailed EF products (e.g., STE1AH), multiplying by sectoral allocations.<br>
```{r, echo=FALSE, fig.cap=""}
knitr::include_graphics(system.file("extdata/4.png", package = "mrprom"))
```
Disaggregate Transport IEA sectors to Transport sector (e.g., GSL ROAD -> PC,PB,GU).<br>
The shares are from Eurostat.
```{r, echo=FALSE, fig.cap=""}
...
## ElecProd
The IEA dataset provides electricity production data for 169 countries and 17 regions up to 2023.<br>
All remaining countries in the 249-country structure used by calcOutput are assigned a value of zero.<br>
The readIEA2025() function imports these data and handles the three available<br>
modes—NonCHP, CHP, and Total—mapping IEA products to OPEN-PROM energy forms (EFs)<br>
and then to the model’s country list.<br>
Depending on the mode, IEA subsets (ELMAINE, ELAUTOE, ELMAINC, ELAUTOC, or ELOUTPUT)<br>
are filtered and mapped to OPEN-PROM EFs.<br>
When mode == "NonCHP", the function uses the ELMAINE and ELAUTOE subsets, which correspond to electricity production<br> excluding combined heat and power (CHP).<br>
For mode == "CHP", the subsets ELMAINC and ELAUTOC are used, capturing only<br>
CHP-related electricity generation.<br>
When mode == "Total", the broader ELOUTPUT subset is selected, providing<br>
total electricity production without distinguishing between CHP and non-CHP sources.<br>
For NonCHP and Total, production is further disaggregated to technologies (PGALL)<br>
using installed capacity shares, while for CHP, production is allocated<br>
via a direct CHP-to-EF mapping.<br>
Missing values are handled by filling zeros.<br>
The processed dataset provides GWh electricity production per region, period,<br>
and technology, and downstream regional aggregation is performed in fullOPEN_PROM<br>
to generate complete production targets.
```{r, echo=TRUE, eval=FALSE}
# Example: read IEA balances
# data <- readIEA2025()
```
...
## Capacity
ENERDATA provides installed electricity capacity data for 179 countries and 2 regions up to 2021.<br>
To ensure full global coverage, all remaining countries in the 249-country<br>
structure used by calcOutput are assigned zero values.<br>
The readENERDATA() function imports these inputs, maps the available country<br>
and regional entries to the model’s complete country list using the mapping<br>
file prom-enerdata-pgall-mapping.csv, and constructs a full 249-country dataset.<br>
Capacity values are handled according to four modes: NonCHP, CHP, Total, and TotalEff.<br>
For NonCHP and CHP, capacities are derived from historical electricity production (from IEA)<br>
divided by annual hours (8760 h) to obtain GW.<br>
For Total, ENERDATA capacities are used directly.<br>
For TotalEff, capacities are multiplied by availability rates from<br>
calcOutput(type = "IAvailRate", aggregate = FALSE) and converted from MW to GW.<br>
Missing values are filled with zero, and country-complete coverage is ensured.
```{r, echo=TRUE, eval=FALSE}
...
```
Regional aggregation is then performed downstream in fullOPEN_PROM, producing a<br>
consistent, global dataset of installed electricity capacities by region,<br>
...
The model supports three modes—Total, CHP, and DHP—and two flow types, Inp (inputs) and Out (outputs).<br>
...
This workflow constructs a comprehensive electricity capacity dataset<br>
and associated parameters for OPEN-PROM.<br>
Nominal capacities (NominalCapacities) are derived from historical installed<br>
capacities (IInstCapPast, mode = "Total"), while effective capacities<br>
(EffCapacities, mode = "TotalEff") are adjusted by availability rates.<br>
ENERDATA total installed capacities (TOTINSTCAP) are imported,<br>
converted from MW to GW, and combined with nominal capacities to compute<br>
TOTNOMCAP per region and period.<br>
The final dataset merges effective and nominal capacities,<br>
calculates PEAKLOAD as 90% of total capacity and BASELOAD as 35.76% of PEAKLOAD,<br>
and initializes placeholders for CHP, non-CHP, and various steam generation variables.
```{r, echo=TRUE, eval=FALSE}
