...
#' @examples
#' \dontrun{
#' a <- calcOutput(type = "IACTV_OP_GEM", aggregate = TRUE)
#' }
#' @importFrom quitte as.quitte interpolate_missing_periods
#' @importFrom dplyr filter select last group_by mutate

calcIACTV_OP_GEM <- function() {
  
  x <- readSource("OPEN_GEM", convert = TRUE)
  ...
  # Sectoral mapping
  map <- toolGetMapping("OPEN_PROM_GEM_mapping.csv", type = "sectoral", where = "mrprom")
