...
#' @examples
#' \dontrun{
#' a <- readSource("ff55EnergyTransportGHG", convert = FALSE)
#' }
#' 
...
#'
readff55EnergyTransportGHG <- function() {
  ...
  # Remove "Cover" sheet
  sheetNames <- sheetNames[sheetNames != "Cover"]
  
  # Extract region codes (AT, BE, BG, …)
  regions <- unique(sub("_[AB]$", "", sheetNames))
  
  # For each region, read sheet_A and sheet_B and rbind them
  result <- lapply(regions, function(region) {
    
    sheetsToRead <- sheetNames[grepl(paste0("^", region, "_[AB]$"), sheetNames)]
    
    # Read both sheets
    dfs <- lapply(sheetsToRead, function(s) {
      read_excel(path = file, sheet = s)
    })
    
    # Combine
    combined <- bind_rows(dfs, .id = "sheetId")
  ...
  for (i in 1:length(result)) {
    ...
    # Drop row 1 and other specified rows
    y <- y[-c(1, 8, 19, 33, 35, 42, 54, 56, 73, 82, 92, 108, 112, 118, 120:134,
              153, 171, 172, 207, 214, 232:242), ]
