...
calcIDataElecSteamGen <- function() {
  ...
  x1 <- x[, , "Total electricity capacity coal, lignite (multifuel included)"]
  x2 <- x[, , "Single fired electricity capacity lignite"]
  x3 <- x[, , "Total electricity capacity gas (multifuel oil/gas included)"]
  x4 <- x[, , "Installed capacity in combined cycles"]
  
  x[, , "Total electricity capacity coal, lignite (multifuel included)"] <- ifelse(is.na(x1), 0, x1) - ifelse(is.na(x2), 0, x2)
  x[, , "Total electricity capacity gas (multifuel oil/gas included)"] <- ifelse(is.na(x3), 0, x3) - ifelse(is.na(x4), 0, x4)
