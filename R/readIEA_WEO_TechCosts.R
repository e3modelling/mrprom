...
#' Available types are: Renewables, Nuclear, Fossil fuels equipped with CCUS,
#' Coal, Gas and convert it to a magpie object.
#' 
...
#' @examples
#' \dontrun{
#' a <- readSource("IEA_WEO_TechCosts")
#' }
#'
...
#'
readIEA_WEO_TechCosts <- function() {
  ...
  data["scenario"] <-  NA
    ...
    # Convert to magpie object
    Nuclear <- as.quitte(data) %>% as.magpie()
    ...
    # Convert to magpie object
    Gas <- as.quitte(data) %>% as.magpie()
    
    x <- mbind(Renewables, Nuclear, Fossil, Coal, Gas)
